...
export async function up({ db, payload: _payload, req: _req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`leads_services_requested_order_idx\` ON \`leads_services_requested\` (\`order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`leads_services_requested_parent_idx\` ON \`leads_services_requested\` (\`parent_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`leads_quotation_parent_id_idx\` ON \`leads_quotation\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`leads_assigned_employee_idx\` ON \`leads\` (\`assigned_employee_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`storage_tags_parent_id_idx\` ON \`storage_tags\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`form_fields_options_order_idx\` ON \`form_fields_options\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`form_fields_options_parent_id_idx\` ON \`form_fields_options\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`form_fields_updated_at_idx\` ON \`form_fields\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`form_fields_created_at_idx\` ON \`form_fields\` (\`created_at\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`quotations_categories_items_order_idx\` ON \`quotations_categories_items\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`quotations_categories_items_parent_id_idx\` ON \`quotations_categories_items\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`quotations_categories_order_idx\` ON \`quotations_categories\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`quotations_categories_parent_id_idx\` ON \`quotations_categories\` (\`_parent_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_employees_id_idx\` ON \`payload_locked_documents_rels\` (\`employees_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_storage_id_idx\` ON \`payload_locked_documents_rels\` (\`storage_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_services_id_idx\` ON \`payload_locked_documents_rels\` (\`services_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_form_fields_id_idx\` ON \`payload_locked_documents_rels\` (\`form_fields_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_quotations_id_idx\` ON \`payload_locked_documents_rels\` (\`quotations_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_notes_id_idx\` ON \`payload_locked_documents_rels\` (\`notes_id\`);`,
  )
