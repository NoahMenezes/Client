...
import { updateQuotationById } from '@/app/actions/quotations'
...
interface QuotationData {
  id: number | string
  title: string
  lead: { id: number | string; fullName: string } | null
  leadId: string
  status: string
...
export default function EditQuotationCategoryForm({ quotation, leads }: Props) {
  const [isPending, startTransition] = useTransition()
  const [error, setError] = useState('')
  const [title, setTitle] = useState(quotation.title)
  const [leadId, setLeadId] = useState(quotation.leadId)
  const [agencyFeePercent, setAgencyFeePercent] = useState(quotation.agencyFeePercent)
  ...
  const [status, setStatus] = useState<'draft' | 'sent' | 'approved' | 'rejected'>(
    quotation.status as 'draft' | 'sent' | 'approved' | 'rejected',
  )
  ...
  const subTotal = categories.reduce(
    (sum, cat) => sum + cat.items.reduce((s, item) => s + (item.amount || 0) * (item.quantity || 1), 0),
    0,
  )
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await updateQuotationById(null, fd)
    ...
    <div className="flex flex-1 flex-col min-h-screen bg-gray-50">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b bg-white">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold">Edit Quotation – {quotation.title}</h1>
        ...
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden mb-5">
          <div className="flex items-center justify-between px-5 py-4 border-b">
            ...
            <p className="text-xs text-gray-400">
              Last Edited by {quotation.lead?.fullName ?? 'Admin'} on{' '}
          ...
          {categories.map((cat, ci) => (
            <div key={ci} className="border-b last:border-0">
              ...
              <div className="flex items-center gap-3 px-5 py-3 bg-gray-50 border-b">
                <IconGripVertical className="h-4 w-4 text-gray-300 shrink-0" />
                <input
                  type="text"
